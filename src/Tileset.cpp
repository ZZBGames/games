...
sf::Vector2f Tileset::getTile(unsigned long index) const
{
    ...
    if (index >= tileCount)
        throw ExceptionBuilder::outOfRangeException("Tile index is out of range", index, 0UL, tileCount);
...
sf::Vector2f Tileset::getTile(unsigned long rowIndex, unsigned long columnIndex) const
{
    if (rowIndex >= m_rowCount)
        throw ExceptionBuilder::outOfRangeException("Row index is out of range", rowIndex, 0UL, m_rowCount);

    if (columnIndex >= m_columnCount)
        throw ExceptionBuilder::outOfRangeException("Column index is out of range", columnIndex, 0UL, m_columnCount);
